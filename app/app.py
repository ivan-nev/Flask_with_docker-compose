...
class AdvertisementView(MethodView):
    ...
    def get(self, adv_id: int):
        with Session() as session:
            adv = get_advertisement(adv_id, session)
            return jsonify({
                'id': adv.id,
                'title': adv.title,
                ...
                return jsonify(
                    {'count': len(users),
                     "users": [
                         {"id": u.id,
                          "name": u.username,
                          "creation_time": u.creation_time} for u in users]
                     }
                )
        else:
            with Session() as session:
...
app.add_url_rule('/users/', view_func=UserView.as_view('users'), methods=['POST', 'GET'])
